...
fn test_2() {
	...
	assert_eq!(run_commands_simple(&cmds).len(), 590784);
}
...
#[test]
fn test_3() {
	let cmds = parse_commands(EXAMPLE_INPUT_3);
	assert_eq!(run_commands_simple(&cmds).len(), 474140);
	assert_eq!(run_commands(&cmds), 2_758_514_936_282_235);
}
